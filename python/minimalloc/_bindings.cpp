...
namespace
{
    ...
    [[nodiscard]] constexpr size_t make_hash(const T &first, const Args &...args) noexcept
    {
...
PYBIND11_MODULE(_minimalloc, m)
{
    py::class_<absl::Duration>(m, "Duration")
        .def(py::init<>())
        .def(py::init([](double s)
                      { return absl::Seconds(s); }),
             "seconds"_a)
        .def_static("from_seconds", [](double s)
                    { return absl::Seconds(s); }, "seconds"_a)
        .def_static("from_milliseconds", [](double ms)
                    { return absl::Milliseconds(ms); }, "milliseconds"_a)
        .def_static("from_microseconds", [](double us)
                    { return absl::Microseconds(us); }, "microseconds"_a)
        .def_static("infinite", absl::InfiniteDuration)
        .def("to_seconds", [](const absl::Duration &d)
             { return absl::ToDoubleSeconds(d); })
        .def("__repr__", [](const absl::Duration &d)
             { return d == absl::InfiniteDuration()
                          ? py::str("Duration.infinite()")
                          : py::str("Duration({} seconds)").format(absl::ToDoubleSeconds(d)); })
        .def(py::self == py::self)
        .def(py::self < py::self)
        .def(py::self <= py::self);
    ...
    py::class_<Interval<int64_t>>(m, "Interval")
        ...
        .def(py::self == py::self)
        .def(py::self < py::self)
        .def("__hash__", [](const Interval<int64_t> &i)
    ...
    py::class_<Gap>(m, "Gap")
        .def(py::init<>())
        .def(py::init<Lifespan, std::optional<Window>>(), "lifespan"_a, "window"_a = std::nullopt)
        .def_readwrite("lifespan", &Gap::lifespan)
        .def_readwrite("window", &Gap::window)
        .def(py::self == py::self)
        .def("__hash__", [](const Gap &g)
             { return make_hash(g.lifespan.lower(), g.lifespan.upper(),
                                g.window ? make_hash(g.window->lower(), g.window->upper()) : 0); })
        .def("__repr__", [](const Gap &g)
             { return py::str("Gap(lifespan={}, window={})").format(g.lifespan, g.window ? py::cast(*g.window) : py::none()); });

    py::class_<Buffer>(m, "Buffer")
        .def(py::init<>())
        .def(py::init([](std::string id, Lifespan lifespan, int64_t size, int64_t alignment,
                         std::vector<Gap> gaps, std::optional<Offset> offset, std::optional<Offset> hint)
                      { return Buffer{std::move(id), lifespan, size, alignment,
                                      std::move(gaps), offset, hint}; }),
             "id"_a, "lifespan"_a, "size"_a, "alignment"_a = 1, "gaps"_a = std::vector<Gap>{},
             "offset"_a = std::nullopt, "hint"_a = std::nullopt)
        ...
        .def("effective_size", [](const Buffer &self, const Buffer &other)
             { return self.effective_size(other); }, "other"_a)
        .def(py::self == py::self)
        .def("__hash__", [](const Buffer &b)
             {
            size_t seed = make_hash(b.id, b.lifespan.lower(), b.lifespan.upper(), 
                                  b.size, b.alignment);
        ...
        .def("__repr__", [](const Buffer &b)
             { return py::str("Buffer(id='{}', lifespan={}, size={}, alignment={}, "
                              "gaps={}, offset={}, hint={})")
                   .format(
                       b.id, b.lifespan, b.size, b.alignment, b.gaps,
    ...
    py::class_<Solution>(m, "Solution")
        ...
        .def(py::self == py::self)
        .def("__hash__", [](const Solution &s)
             {
            size_t seed = 0;
            for (auto off : s.offsets) seed ^= make_hash(off);
            return seed; })
        .def("__repr__", [](const Solution &s)
             { return py::str("Solution(offsets={})").format(s.offsets); });

    py::class_<Problem>(m, "Problem")
        .def(py::init<>())
        .def(py::init<std::vector<Buffer>, Capacity>(), "buffers"_a, "capacity"_a = 0)
        .def_readwrite("buffers", &Problem::buffers)
        .def_readwrite("capacity", &Problem::capacity)
        .def("strip_solution", [](Problem &p)
             { return unwrap_or_none(p.strip_solution()); })
        .def(py::self == py::self)
        .def("__hash__", [](const Problem &p)
             {
        ...
        .def("__repr__", [](const Problem &p)
             { return py::str("Problem(buffers={}, capacity={})").format(p.buffers, p.capacity); });

    py::class_<SolverParams>(m, "SolverParams")
        .def(py::init<>())
        .def_property("timeout", [](const SolverParams &self)
                      { return self.timeout; }, [](SolverParams &self, py::object value)
                      {
                if (py::isinstance<absl::Duration>(value)) {
                    ...
                    throw py::type_error("timeout must be Duration, numeric, or None");
                } })
    ...
    py::class_<PreorderData>(m, "PreorderData")
        .def(py::init<>())
        .def(py::init([](Area area, TimeValue lower, uint64_t overlaps, int sections,
                         int64_t size, int total, TimeValue upper, int64_t width,
                         BufferIdx buffer_idx)
                      {
            PreorderData data;
            data.area = area; data.lower = lower; data.overlaps = overlaps;
            data.sections = sections; data.size = size; data.total = total;
            data.upper = upper; data.width = width; data.buffer_idx = buffer_idx;
            return data; }),
             "area"_a, "lower"_a, "overlaps"_a, "sections"_a, "size"_a,
             "total"_a, "upper"_a, "width"_a, "buffer_idx"_a)
    ...
    py::class_<Solver>(m, "Solver")
        ...
        .def("solve", [](Solver &self, const Problem &problem)
             { return unwrap_or_none(self.Solve(problem)); }, "problem"_a)
        .def("get_backtracks", &Solver::get_backtracks)
        .def("cancel", &Solver::Cancel)
        .def("compute_irreducible_infeasible_subset", [](Solver &self, const Problem &problem)
             { return unwrap_or_none(self.ComputeIrreducibleInfeasibleSubset(problem)); }, "problem"_a);
    ...
    m.def("to_csv", [](const Problem &problem, std::optional<Solution> solution, bool old_format)
          { return solution ? ToCsv(problem, &solution.value(), old_format)
                            : ToCsv(problem, nullptr, old_format); }, "problem"_a, "solution"_a = std::nullopt, "old_format"_a = false);

    m.def("from_csv", [](std::string_view input)
          { return unwrap_or_none(FromCsv(input)); }, "input"_a);
    ...
    py::class_<SectionSpan>(m, "SectionSpan")
        ...
        .def(py::self == py::self)
        .def("__hash__", [](const SectionSpan &s)
        ...
        .def("__repr__", [](const SectionSpan &s)
             { return py::str("SectionSpan(section_range={}, window={})").format(s.section_range, s.window); });

    py::class_<Partition>(m, "Partition")
        .def(py::init<>())
        .def(py::init<std::vector<BufferIdx>, SectionRange>(), "buffer_idxs"_a, "section_range"_a)
        .def_readwrite("buffer_idxs", &Partition::buffer_idxs)
        .def_readwrite("section_range", &Partition::section_range)
        .def(py::self == py::self)
        .def("__hash__", [](const Partition &p)
             {
            size_t seed = make_hash(p.section_range.lower(), p.section_range.upper());
            for (auto idx : p.buffer_idxs) seed ^= make_hash(idx);
            return seed; })
        .def("__repr__", [](const Partition &p)
             { return py::str("Partition(buffer_idxs={}, section_range={})").format(p.buffer_idxs, p.section_range); });

    py::class_<Overlap>(m, "Overlap")
        .def(py::init<>())
        .def(py::init<BufferIdx, int64_t>(), "buffer_idx"_a = -1, "effective_size"_a = 0)
        .def_readwrite("buffer_idx", &Overlap::buffer_idx)
        .def_readwrite("effective_size", &Overlap::effective_size)
        .def(py::self == py::self)
        .def(py::self < py::self)
        .def("__hash__", [](const Overlap &o)
             { return make_hash(o.buffer_idx, o.effective_size); })
        .def("__repr__", [](const Overlap &o)
             { return py::str("Overlap(buffer_idx={}, effective_size={})").format(o.buffer_idx, o.effective_size); });

    py::class_<BufferData>(m, "BufferData")
        .def(py::init<>())
        .def(py::init([](std::vector<SectionSpan> section_spans, std::optional<py::iterable> overlaps)
                      {
            BufferData data;
            ...
            return data; }),
             "section_spans"_a = std::vector<SectionSpan>{}, "overlaps"_a = std::nullopt)
        .def_readwrite("section_spans", &BufferData::section_spans)
        .def_property("overlaps", [](const BufferData &self)
                      {
                py::set py_overlaps;
                for (const auto& overlap : self.overlaps) {
                    py_overlaps.add(py::cast(overlap));
                }
                return py_overlaps; }, [](BufferData &self, const py::iterable &overlaps)
                      {
                self.overlaps.clear();
                for (const auto& item : overlaps) {
        ...
        .def(py::self == py::self)
        .def("__hash__", [](const BufferData &b)
             {
            size_t seed = 0;
            for (const auto& span : b.section_spans) {
                seed ^= make_hash(span.section_range.lower(), span.section_range.upper());
            }
        ...
        .def("__repr__", [](const BufferData &b)
            ...
            return py::str("BufferData(section_spans={}, overlaps={})").format(
                b.section_spans, py_overlaps); });

    py::class_<SweepResult>(m, "SweepResult")
        .def(py::init<>())
        .def(py::init([](std::optional<py::list> sections, std::vector<Partition> partitions,
            ...
            return result; }),
             "sections"_a = std::nullopt, "partitions"_a = std::vector<Partition>{},
             "buffer_data"_a = std::vector<BufferData>{})
        .def_property("sections", [](const SweepResult &self)
                      {
                py::list sections;
                ...
                return sections; }, [](SweepResult &self, const py::list &sections)
                      {
                self.sections.clear();
    ...
    py::class_<SweepPoint>(m, "SweepPoint")
        ...
        .def(py::init<BufferIdx, TimeValue, SweepPointType, Window, bool>(),
             "buffer_idx"_a, "time_value"_a, "point_type"_a, "window"_a, "endpoint"_a = false)
        ...
        .def(py::self == py::self)
        .def(py::self < py::self)
        .def("__hash__", [](const SweepPoint &p)
             { return make_hash(p.buffer_idx, p.time_value, static_cast<int>(p.point_type),
                                p.window.lower(), p.window.upper(), p.endpoint); })
        .def("__repr__", [](const SweepPoint &p)
             { return py::str("SweepPoint(buffer_idx={}, time_value={}, point_type={}, "
                              "window={}, endpoint={})")
                   .format(
                       p.buffer_idx, p.time_value,
